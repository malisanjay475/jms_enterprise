...
// Security Headers
// app.use(helmet({
//   contentSecurityPolicy: false, // Disable CSP for now to allow inline scripts/styles (dev mode)
//   crossOriginResourcePolicy: false,
//   crossOriginEmbedderPolicy: false,
//   originAgentCluster: false // Sometimes causes issues with IP access
// }));
// GZIP Compression
...
function getWriteFactoryHeaderState(req) {
  ...
  const fallbackFactoryId = getFactoryId(req);
  return {
    requestedAll: false,
...
app.post('/api/hr/operators', async (req, res) => {
  ...
  try {
    ...
    if (photo_base64 && photo_base64.includes('base64')) {
      ...
      const uploadsDir = path.join(__dirname, 'public/uploads/operators');
