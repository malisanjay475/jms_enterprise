...
function buildReadme({ localServer, mainServerUrl }) {
  return [
    ...
    '8. (Recommended) Run REGISTER_AUTOSTART.bat so the server starts at every Windows boot. It will',
    '   request Administrator rights automatically (needed to install the boot-time task).',
    '',
    '## Auto-update',
    ...
    '- After running REGISTER_AUTOSTART.bat, it starts automatically at Windows boot - even before',
    '  anyone logs in - and Windows restarts the task if it ever stops.',
    '',
    '## Result',
...
function buildSupervisorJs() {
  return [
    ...
    "const AUTO_UPDATE_INTERVAL_MS = 5 * 60 * 1000; // check every 5 minutes",
    '',
    ...
    '  // shell:true is REQUIRED on Windows: since the CVE-2024-27980 patch, Node',
    "  // refuses to spawn .cmd/.bat files with shell:false and throws EINVAL, so",
    '  // npm.cmd install silently fails and every auto-update rolls back.',
    ...
    "  const npm = process.platform === 'win32' ? 'npm.cmd' : 'npm';",
    "  const result = spawnSync(npm, ['install', '--production', '--no-audit'], {",
    "    cwd: clientBridgeDir, stdio: 'inherit', shell: process.platform === 'win32'",
    '  });',
    '  if (result.status !== 0) {',
    ...
    '}',
    '',
    '// ── Auto-update: download changed files from main server ────────────────',
    'function loadEnvValue(key) {',
    "  var envPath = path.join(backendDir, '.env');",
    ...
    '}',
    '',
    'function httpGetAuth(url, nodeKey) {',
    '  return new Promise(function(resolve, reject) {',
    '    var parsed = new URL(url);',
    "    var lib = parsed.protocol === 'https:' ? https : http;",
    "    var opts = { hostname: parsed.hostname, port: parsed.port, path: parsed.pathname + parsed.search, headers: { 'x-node-key': nodeKey } };",
    ...
    "  var mainUrl    = loadEnvValue('MAIN_SERVER_URL');",
    "  var nodeId     = loadEnvValue('LOCAL_SERVER_NODE_ID');",
    "  var nodeKey    = loadEnvValue('LOCAL_SERVER_NODE_KEY');",
    '  if (!mainUrl || !nodeId || !nodeKey) return;',
    '',
    "  var base = mainUrl.replace(/\\/+$/, '');",
    "  var manifestUrl = base + '/api/local-servers/' + nodeId + '/file-manifest';",
    '',
    '  httpGetAuth(manifestUrl, nodeKey).then(function(res) {',
    '    if (res.status !== 200) return;',
    '    var data;',
    '    try { data = JSON.parse(res.body.toString()); } catch(e) { return; }',
    '    if (!data.ok || !data.manifest) return;',
    '',
    '    var manifest = data.manifest;',
    '    var filePaths = Object.keys(manifest);',
    '    var toUpdate = [];',
    '    for (var i = 0; i < filePaths.length; i++) {',
    '      var relPath = filePaths[i];',
    '      var localPath = path.join(backendDir, relPath);',
    '      var localHash = sha256File(localPath);',
    '      if (localHash !== manifest[relPath]) toUpdate.push(relPath);',
    '    }',
    '',
    '    if (toUpdate.length === 0) return;',
    "    console.log('[AutoUpdate] ' + toUpdate.length + ' file(s) need updating: ' + toUpdate.slice(0, 5).join(', ') + (toUpdate.length > 5 ? '...' : ''));",
    '',
    '    var chain = Promise.resolve();',
    '    toUpdate.forEach(function(relPath) {',
    '      chain = chain.then(function() {',
    '        var fileUrl = base + \'/api/local-servers/\' + nodeId + \'/file?p=\' + encodeURIComponent(relPath);',
    '        return httpGetAuth(fileUrl, nodeKey).then(function(fr) {',
    '          if (fr.status !== 200) {',
    "            console.warn('[AutoUpdate] Failed to download ' + relPath + ' (HTTP ' + fr.status + ')');",
    '            return;',
    '          }',
    '          var localPath = path.join(backendDir, relPath);',
    '          fs.mkdirSync(path.dirname(localPath), { recursive: true });',
    '          fs.writeFileSync(localPath, fr.body);',
    ...
    "      console.log('[AutoUpdate] All files updated. Restarting backend...');",
    '      if (backendProcess) {',
    '        backendProcess.kill();   // supervisor will restart it automatically',
    '      }',
    '    });',
    '  }).catch(function(err) {',
    ...
    '}',
    '',
    'function startManagedProcess(label, cwd, scriptName, enabled, onSpawn) {',
    '  if (!enabled) return;',
    '  var failCount = 0;',
    '  function launch() {',
    ...
    "startManagedProcess('Backend', backendDir, 'server.js', backendEnabled, function(child) { backendProcess = child; });",
    '',
    "var bridgeEnabled = fs.existsSync(path.join(clientBridgeDir, 'bridge.js'));",
    'if (bridgeEnabled) {',
    '  bridgeEnabled = ensureClientBridgeDeps();',
    '}',
    "startManagedProcess('Client Bridge', clientBridgeDir, 'bridge.js', bridgeEnabled, null);",
    '',
    '// Auto-update: check for code changes from main server every 5 minutes',
    "console.log('[AutoUpdate] Will check for updates every ' + (AUTO_UPDATE_INTERVAL_MS / 60000) + ' min');",
    'setTimeout(function() {',
    '  runAutoUpdate();',
...
function buildAutostartBat() {
  return [
    ...
    'echo ================================================',
    'echo.',
    'echo This registers JMS Local Server to start automatically at',
    'echo Windows boot - even before anyone logs in - and to self-heal',
    'echo if it ever stops.',
    'echo.',
    ':: A SYSTEM boot task needs Administrator rights - self-elevate if needed.',
    ...
    '  exit /b 1',
    ')',
    'if not exist "%~dp0REGISTER_AUTOSTART.js" (',
    '  echo [ERROR] REGISTER_AUTOSTART.js not found.',
    '  pause',
    '  exit /b 1',
    ')',
    'node "%~dp0REGISTER_AUTOSTART.js"',
    'echo.',
    'pause'
  ].join('\r\n');
}

function buildAutostartJs() {
  return [
    "'use strict';",
    '',
    "const fs = require('fs');",
    ...
    '',
    'const rootDir = __dirname;',
    "const supervisorPath = path.join(rootDir, 'LOCAL_SERVER_SUPERVISOR.js');",
    'const nodeExe = process.execPath;',
    "const TASK_NAME = 'JMS Local Server';",
    '',
    ...
    '}',
    '',
    '// Build a Task Scheduler definition that:',
    '//  - starts at boot under the SYSTEM account (no login needed)',
    '//  - restarts the task every minute if it ever stops (self-heal)',
    '//  - has no execution time limit and ignores battery state',
    'function buildTaskXml() {',
    '  const command = escapeXml(nodeExe);',
    "  const args = escapeXml('\"' + supervisorPath + '\"');",
    '  const workDir = escapeXml(rootDir);',
    '  return [',
    '    \'<?xml version="1.0" encoding="UTF-16"?>\',',
    '    \'<Task version="1.2" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">\',',
    "    '  <RegistrationInfo>',",
    "    '    <Description>JMS Local Server supervisor - starts at boot (no login needed) and self-heals.</Description>',",
    "    '  </RegistrationInfo>',",
    "    '  <Triggers>',",
    ...
    '}',
    '',
    'function registerViaSchtasks() {',
    "  const xmlPath = path.join(os.tmpdir(), 'jms-local-server-task.xml');",
    "  fs.writeFileSync(xmlPath, '\\ufeff' + buildTaskXml(), { encoding: 'utf16le' });",
    "  const result = spawnSync('schtasks', ['/create', '/tn', TASK_NAME, '/xml', xmlPath, '/ru', 'SYSTEM', '/f'], {",
    ...
    '}',
    '',
    'function registerViaStartupFolder() {',
    ...
    "  const batPath = path.join(startupDir, 'JMS_Local_Server.bat');",
    '  const lines = [',
    "    '@echo off',",
    '    \'start "" /b "\' + nodeExe + \'" "\' + supervisorPath + \'"\'',
    "  ].join('\\r\\n');",
    "  fs.writeFileSync(batPath, lines, 'utf8');",
    '  return true;',
    '}',
    '',
    'function main() {',
    '  if (!fs.existsSync(supervisorPath)) {',
    "    console.error('[ERROR] LOCAL_SERVER_SUPERVISOR.js not found next to this script.');",
    '    process.exitCode = 1;',
    '    return;',
    '  }',
    '  console.log(\'[INFO] Registering "\' + TASK_NAME + \'" to start at Windows boot (no login required)...\');',
    '  if (registerViaSchtasks()) {',
    "    console.log('[OK] Registered as a SYSTEM boot task. The server now starts on every reboot,');",
    "    console.log('     before anyone logs in, and Windows restarts it if it stops.');",
    '    return;',
    '  }',
    "  console.warn('[WARN] Boot-task registration failed (are you running as Administrator?).');",
    "  console.warn('[WARN] Falling back to the per-user Startup folder (requires login to start)...');",
    '  if (registerViaStartupFolder()) {',
    "    console.log('[OK] Added to the Windows Startup folder. It will start when this user logs in.');",
    '    return;',
    '  }',
    "  console.error('[ERROR] Could not register autostart by any method.');",
    '  process.exitCode = 1;',
    '}',
    '',
    'main();',
...
function buildProvisioningPackage({ localServer, nodeKey, mainServerUrl, syncApiKey }) {
  ...
  zip.addFile(
    path.posix.join(packageRoot, 'REGISTER_AUTOSTART.js'),
    Buffer.from(buildAutostartJs(), 'utf8')
  );
