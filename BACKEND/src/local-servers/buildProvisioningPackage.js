...
function buildReadme({ localServer, mainServerUrl }) {
  return [
    ...
    '5. If another app already uses port 3000, keep the default local JMS port 3001 or enter any free port.',
...
function buildInstallerJs() {
  return [
    ...
    "    envMap.set('PORT', await askQuestion(rl, 'Local JMS app port', envMap.get('PORT') || '3001'));",
    "    envMap.set('HTTPS_PORT', await askQuestion(rl, 'Local JMS HTTPS port (only used when HTTPS is enabled)', envMap.get('HTTPS_PORT') || '3444'));",
