...
function buildSupervisorJs() {
  return [
    ...
    '',
    '// Auto-install BACKEND node_modules when package.json has changed or node_modules is missing.',
    '// Uses a marker file (.npm-install-marker) that records the mtime of the last package.json',
    '// that was installed. On a fresh extract or after an auto-update that adds new packages,',
    '// the marker will be older than (or absent from) package.json and npm install re-runs.',
    'function ensureBackendDeps() {',
    "  const pkgPath  = path.join(backendDir, 'package.json');",
    "  const nmPath   = path.join(backendDir, 'node_modules');",
    "  const marker   = path.join(backendDir, 'node_modules', '.npm-install-marker');",
    '  if (!fs.existsSync(pkgPath)) return true; // nothing to install',
    '  var pkgMtime = fs.statSync(pkgPath).mtimeMs;',
    '  var markerMtime = 0;',
    '  try { markerMtime = parseFloat(fs.readFileSync(marker, "utf8")); } catch (e) {}',
    '  if (fs.existsSync(nmPath) && markerMtime >= pkgMtime) return true; // already up to date',
    "  console.log('[Supervisor] BACKEND dependencies outdated or missing — running npm install...');",
    "  const npm = process.platform === 'win32' ? 'npm.cmd' : 'npm';",
    "  const result = spawnSync(npm, ['install', '--production', '--no-audit'], {",
    "    cwd: backendDir, stdio: 'inherit', shell: false",
    '  });',
    '  if (result.status !== 0) {',
    "    console.error('[Supervisor] npm install for BACKEND failed — server may crash on missing modules.');",
    '    return false;',
    '  }',
    '  try { fs.writeFileSync(marker, String(pkgMtime), "utf8"); } catch (e) {}',
