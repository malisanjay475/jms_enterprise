                            ...
                            lines[lineName].forEach(machine => {
                                ...
                                shiftsToRender.forEach(rowShift => {
                                    ...
                                    distinctMoulds.forEach((m, mIdx) => {
                                        ...
                                        // Cycle Time (KAN-65): STD from mould master; Avg CT = net run time (s) ÷ shots.
                                        // Operators often don't record the `shots` field, so fall back to deriving
                                        // shots from produced pieces ÷ cavity (each shot makes `cavity` pieces) —
                                        // otherwise Avg CT shows "-" on machines that clearly ran.
                                        const stdCt = parseFloat(m.details?.std_cycle_time || 0);
                                        const ctCav = parseFloat(m.details?.act_cavity || m.details?.std_cavity || 0);
                                        const effShots = sumShots > 0 ? sumShots : ((totalPcs > 0 && ctCav > 0) ? (totalPcs / ctCav) : 0);
                                        const avgCt = (effShots > 0 && runTimeMins > 0) ? (runTimeMins * 60) / effShots : 0;
