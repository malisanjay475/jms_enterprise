...
function buildSupervisorScript() {
  return [
    ...
    '// Uses the same signature as the backend updater (BACKEND/services/dependencySignature.js),',
    '// since both write the same marker; falls back to the raw hash on an older BACKEND.',
    'function getDependencySignature(rootDir) {',
    '  try {',
    "    return require(path.join(backendDir, 'services', 'dependencySignature.js')).getDependencySignature(rootDir);",
    '  } catch (_e) { /* older BACKEND without the shared module */ }',
    ...
    "    // npm.cmd cannot be spawned without a shell on Windows (EINVAL).",
